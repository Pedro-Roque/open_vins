...
void TrackSIFT::feed_new_camera(const CameraData &message) {
...
void TrackSIFT::feed_monocular(const CameraData &message, size_t msg_id) {
  ...
  std::cout << "Perform detection again..." << std::endl;
  ...
  std::cout << "Do robust match..." << std::endl;
  std::cout << "We are really here..." << std::endl;
...
void TrackSIFT::feed_stereo(const CameraData &message, size_t msg_id_left, size_t msg_id_right) {
...
void TrackSIFT::perform_detection_monocular(const cv::Mat &img0, const cv::Mat &mask0, std::vector<cv::KeyPoint> &pts0,
  ...
  this->sift0->compute(img0, pts0_ext, desc0_ext);
...
void TrackSIFT::perform_detection_stereo(const cv::Mat &img0, const cv::Mat &img1, const cv::Mat &mask0, const cv::Mat &mask1,
  ...
  parallel_for_(cv::Range(0, 2), LambdaBody([&](const cv::Range &range) {
                  for (int i = range.start; i < range.end; i++) {
                    ...
                    (is_left ? sift0 : sift1)->compute(is_left ? img0 : img1, is_left ? pts0_ext : pts1_ext, is_left ? desc0_ext : desc1_ext);
                  }
                }));
...
void TrackSIFT::robust_match(const std::vector<cv::KeyPoint> &pts0, const std::vector<cv::KeyPoint> &pts1, const cv::Mat &desc0,
                                   const cv::Mat &desc1, size_t id0, size_t id1, std::vector<cv::DMatch> &matches) {

  // Our 1to2 and 2to1 match vectors
  std::cout << "-RB: Matching descriptors..." << std::endl;
  ...
  std::cout << " Doing RANSAC testing..." << std::endl;
...
void TrackSIFT::robust_ratio_test(std::vector<std::vector<cv::DMatch>> &matches) {
...
void TrackSIFT::robust_symmetry_test(std::vector<std::vector<cv::DMatch>> &matches1, std::vector<std::vector<cv::DMatch>> &matches2,
