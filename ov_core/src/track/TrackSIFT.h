...
#ifndef OV_CORE_TRACK_SIFT_H
#define OV_CORE_TRACK_SIFT_H
 ...
 * Right now we use SIFT descriptors as we have found it is the fastest when computing descriptors.
 * Tracks are then rejected based on a ratio test and ransac.
 */
class TrackSIFT : public TrackBase {
...
public:
   ...
   */
  explicit TrackSIFT(std::unordered_map<size_t, std::shared_ptr<CamBase>> cameras, int numfeats, int numaruco, bool stereo,
...
protected:
  ...
  // Our SIFT extractor
  cv::Ptr<cv::SIFT> sift0 = cv::SIFT::create();
